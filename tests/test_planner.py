...
def test_non_regulator_sources_are_unaffected_by_the_regulator_map(registry):
    # NIRF and NTA serve every course area; only regulators are filtered.
    # NSP was dropped from this assertion when Scholarships was retired -- it
    # was that segment's only source, so nothing routes to it now.
    planned = {i.source_id for i in plan_course(_course(), registry)}
    assert {"NIRF", "NTA"} <= planned
